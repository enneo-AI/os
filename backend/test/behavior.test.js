...
test('marketplace connections stay dormant until an accessible Space activates them', () => {
  ...
  assert.match(frontendHtml, /<h2>Marketplace /)
  assert.match(frontendHtml, /class="head-info"/)
...
test('long Enni turns are bounded, never persist empty, and drafts survive reloads', () => {
  const agentSource = readFileSync(join(here, '../src/agent.js'), 'utf8')
  const indexSource = readFileSync(join(here, '../src/index.js'), 'utf8')
  const frontendSource = readFileSync(join(here, '../../frontend/app.js'), 'utf8')
  const frontendHtml = readFileSync(join(here, '../../frontend/index.html'), 'utf8')

  assert.match(agentSource, /const MAX_TOOL_CALLS/)
  assert.match(agentSource, /const MAX_TURN_MS/)
  assert.match(agentSource, /const FINALIZATION_TIMEOUT_MS/)
  assert.match(agentSource, /response\.stop_reason === 'max_tokens'/)
  assert.match(agentSource, /if \(!finalText\.trim\(\) && !aborted\)/)
  assert.match(agentSource, /Ich konnte diesen Arbeitslauf nicht zuverlässig abschließen/)
  assert.match(agentSource, /reason: 'empty_response_fallback'/)
  assert.match(indexSource, /const persistedAssistantText = result\.text\?\.trim\(\)/)
  assert.doesNotMatch(indexSource, /content: result\.text \|\| \(result\.aborted \? '_Gestoppt\._' : ''\)/)

  assert.match(frontendSource, /const DRAFT_PREFIX = 'enneo:draft:v1'/)
  assert.match(frontendSource, /function restoreMainDraft/)
  assert.match(frontendSource, /function restoreThreadDraft/)
  assert.match(frontendSource, /'X-Enneo-Client-Version': APP_ASSET_VERSION/)
  assert.match(frontendSource, /reportClientError\(error, 'thread_stream'/)
  assert.match(frontendHtml, /max-height:min\(54vh,720px\);overflow:auto/)
})

test('running Enni turns survive navigation and reopen their Pod thread', () => {
  const indexSource = readFileSync(join(here, '../src/index.js'), 'utf8')
  const progressSource = readFileSync(join(here, '../src/progress.js'), 'utf8')
  const frontendSource = readFileSync(join(here, '../../frontend/app.js'), 'utf8')
  const frontendHtml = readFileSync(join(here, '../../frontend/index.html'), 'utf8')
  const migration = readFileSync(join(here, '../../supabase/migrations/20260720154855_persistent_conversation_runs.sql'), 'utf8')

  assert.match(migration, /create table if not exists public\.conversation_runs/)
  assert.match(migration, /alter table public\.conversation_runs enable row level security/)
  assert.match(migration, /public\.is_pod_visible\(c\.pod_id\)/)
  assert.match(migration, /grant select on public\.conversation_runs to authenticated/)
  assert.match(migration, /alter publication supabase_realtime add table public\.conversation_runs/)
  assert.match(indexSource, /progress\.start\(\{/)
  assert.match(indexSource, /threadRootId: responseThreadRootId/)
  assert.match(indexSource, /clearConversationRun/)
  assert.match(progressSource, /conversation_runs'\)\.upsert/)
  assert.match(progressSource, /response_text: state\.text/)
  assert.match(frontendSource, /c\.working && c\.active_run\?\.thread_root_id/)
  assert.match(frontendSource, /renderLiveProgressIfWorking\(currentConv, box, rootId\)/)
  assert.match(frontendSource, /table: 'conversation_runs'/)
  assert.match(frontendSource, /Enni arbeitet im Thread/)
  assert.match(frontendHtml, /\.live-run-status/)
  assert.match(frontendHtml, /\.pod-work-label/)
})
