...
- **Backend** `backend/`: Node 22 + Express + `@anthropic-ai/sdk` (manueller Tool-Loop, KEIN Agent-SDK — Begründung in `backend/README.md`). SSE-Streaming. Endpoints: `POST /api/chat` (message, conversation_id?, model?, attachments?, pod_id?, thread_root_id?), `POST /api/compact`, `POST /api/conversations/:id/read`, `GET /health`. Markiert eine Pod-Hauptnachricht `@enni`, landet Ennis Antwort in ihrem Thread; danach klassifiziert Haiku jede Thread-Antwort zurückhaltend und ruft den vollen Agenten nur bei echtem Antwortbedarf.
