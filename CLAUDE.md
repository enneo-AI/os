...
**Was:** Internes AI-Workbench-System für das enneo-Team (~20 MA). Ein Agent (Enni) + Wiki/Spaces (Notion-Ersatz) + Pods (Projekt-Räume) + Admin. Owner: Aleksa (AI Implementation Manager bei enneo). **Stand: produktiv im Team-Test auf `https://os.enneo.ai`.**
...
|---|---|---|
| Frontend | `https://os.enneo.ai` | **automatisch bei git push** (GitHub-Connect, `netlify.toml`: publish=frontend + SPA-Redirects) |
...
- **Frontend** `frontend/`: Vanilla HTML+JS ohne Build (esm.sh für supabase-js/marked/DOMPurify). Konfiguration in `config.js` (Anon-Key ist public). SPA-Routing über History API: `/chat`, `/chat/:id`, `/spaces`, `/spaces/tools`, `/spaces/connections`, `/admin`, `/pod/:id`. Pod-Konversationen haben Slack-artige Seiten-Threads; `?thread=<message-id>` öffnet einen Thread direkt.
- **Backend** `backend/`: Node 22 + Express + `@anthropic-ai/sdk` (manueller Tool-Loop, KEIN Agent-SDK — Begründung in `backend/README.md`). SSE-Streaming. Endpoints: `POST /api/chat` (message, conversation_id?, model?, attachments?, pod_id?, thread_root_id?), `POST /api/compact`, `POST /api/conversations/:id/read`, `GET /health`. Markiert eine Pod-Hauptnachricht `@enni`, landet Ennis Antwort in ihrem Thread; danach klassifiziert Haiku jede Thread-Antwort zurückhaltend und ruft den vollen Agenten nur bei echtem Antwortbedarf. Agentenläufe haben harte Tool-/Zeitbudgets und eine garantierte Abschlusssynthese; leere Assistant-Antworten werden weder erzeugt noch persistiert.
- **Persistenter Live-Status:** `conversation_runs` ist die serverseitige Source of Truth für laufende Enni-Turns (Phase, Thinking-/Antwort-Snapshot, Tools, Pod/Thread-Root, Zeitstempel). Das Backend schreibt ausschließlich per Service Role, RLS begrenzt Reads auf zugängliche Konversationen, Supabase Realtime aktualisiert die UI. Navigation oder Reload dürfen einen Run nicht aus der Oberfläche verlieren; beim Pod-Reentry wird der zugehörige Thread automatisch wieder geöffnet.
- **Enni-Tools:** `wiki_semantic_search` (RAG, pgvector Top-8-Chunks — IMMER zuerst), `wiki_search`/`wiki_list_pages`/`wiki_read_page`, GitLab read-only (`gitlab_search_projects/_search_code/_read_file/_list_merge_requests`). In Pods kommen read-only `pod_*`-Tools für Aufgaben, Dateien, Konversationen sowie Notizen/Meeting-Transkripte hinzu; lange Transkripte werden erst nach `pod_list_notes` gezielt über `pod_read_note` geladen. Der teamweite Skill `/ux-ui-engineering` hat zwei serverseitig getrennte Modi: Members nur eigene `ui_change_requests`; aktive Admins zusätzlich Request-Management und GitLab-Writes ausschließlich auf freigegebenen `enni/ui-*`-Branches im `enneo`-Namespace. Kein Default-Branch-Write, Merge oder Auto-Merge.
- **Marketplace / Connector-Autorisierung:** Der Marketplace speichert Connections, er autorisiert Enni nicht. Externe Connectoren werden in `space_connections` ausschließlich als `connector:<uuid>` aktiviert. `backend/src/connector-access.js` erlaubt sie über Open Spaces für alle aktiven Accounts und über Restricted Spaces nur für Ersteller/ausdrücklich eingeladene Mitglieder; Admin-Status allein ist kein Restricted-Zugriff. `personal/team/proposed` steuert nur Katalogsichtbarkeit. Neue Connections niemals automatisch einem Space zuordnen.
- **Remote MCPs:** Generic Connector unterstützt `mcp_none`, `mcp_bearer` und `mcp_x_api_key`; Header niemals heuristisch vertauschen. Offizielle OAuth-MCPs laufen über `backend/src/mcp-oauth.js` mit Discovery, Dynamic Client Registration, PKCE und Refresh (`mcp_oauth`; zuerst Lemlist). Persönliche Connections gehören immer dem eingeloggten Account – auch Admins können nicht stellvertretend verbinden; gemeinsame Service-Connections erfordern explizit `scope=team`. Tokens, OAuth-Client-Information und PKCE-Verifier werden mit `encryptSecret()` verschlüsselt. Research-Blueprints kommen ausschließlich aus dem erzwungenen `submit_blueprint`-Tool, nicht aus frei geparstem JSON. Jeder veröffentlichte Blueprint braucht eine HTTPS-Logo-/Favicon-URL und wird ohne Sonderrubrik direkt als `Read only` oder `Read & Write` in den bestehenden Marketplace einsortiert. Vor jedem MCP-Call wird die aktuelle Space-Berechtigung erneut geprüft.
...
`profiles` (is_admin, primäre `department`, alle `departments[]`) · `contexts` (personal/team; `personal_profile` ohne Admin-Bypass) · `skill_contexts` (required/optional) · `conversations` (user_id, pod_id) · `messages` (role inkl. compaction, thinking, tool_calls, attachments, author_id, `thread_root_id`; Threads nur in Pods und nur an User-Root-Nachrichten) · `conversation_runs` (dauerhafter Live-Status je laufendem Chat-/Thread-Turn; clientseitig read-only) · `llm_usage` · `wiki_pages` (space_id) · `wiki_chunks` (pgvector 384, RPC `match_wiki_chunks`) · `knowledge_updates` · `ui_change_requests` (Member-eigene UX/UI-Anfragen, Admin-Review/Umsetzung) · `spaces`/`space_members`/`space_connections` (Dust-Spaces) · `pods` (Project Pulse: project_status/current_focus/target_date) · `pod_members` · `pod_tasks` (Beschreibung, Priorität, offen/in Arbeit/blockiert/erledigt) · `pod_task_comments` · `pod_files` (+ Storage-Bucket `pod-files`) · `pod_notes` (kurze Notizen und Meeting-Transkripte, Pod-Mitglieder lesen, Autor bearbeitet/löscht) · Helper `is_pod_visible()` SECURITY DEFINER. Restricted Pods sind ausnahmslos invitation-only, auch für Admins.
...
4. Project Pulse, Aufgabenliste, Kanban-Board sowie gemeinsame Pod-Notizen und Meeting-Transkripte sind live; Meilensteine und strukturierte Projektentscheidungen sind bewusst spätere Ausbaustufen.
